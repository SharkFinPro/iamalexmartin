...
// stored on RichText fields and consumed by RichTextWidget) and the contentEditable
...
//
// Images are represented as Hygraph `image` nodes (self-contained: src + handle +
// dimensions + mimeType), exactly what the renderer's default `img` handler and
// the rest of the CMS already expect — no separate `references` array needed.
...
export function imageNodeHtml(node: AstNode): string {
  ...
  const attrs = [
    ...
    node.handle != null ? `data-handle="${escapeAttr(String(node.handle))}"` : "",
    node.width != null ? `data-width="${escapeAttr(String(node.width))}"` : "",
    node.height != null ? `data-height="${escapeAttr(String(node.height))}"` : "",
    node.mimeType ? `data-mime="${escapeAttr(String(node.mimeType))}"` : "",
...
function serializeInline(nodes: ChildNode[], marks: Marks): AstNode[] {
  ...
  for (const child of nodes) {
    ...
    switch (el.tagName.toLowerCase()) {
      ...
      default: {
        // span / font / unknown inline wrapper: detect any style-based marks
        // (in case the browser emitted CSS rather than tags) and unwrap.
...
function figureToImageNode(fig: HTMLElement): AstNode | null {
  ...
  const handle = fig.getAttribute("data-handle");
  const width = fig.getAttribute("data-width");
  const height = fig.getAttribute("data-height");
  const mime = fig.getAttribute("data-mime");
  const title = fig.getAttribute("data-title");
  const alt = fig.getAttribute("data-alt");
  if (handle) node.handle = handle;
  if (width) node.width = Number(width);
  if (height) node.height = Number(height);
  if (mime) node.mimeType = mime;
...
function blockToAst(el: HTMLElement, out: AstNode[]): void {
  switch (el.tagName.toLowerCase()) {
    ...
    case "div": {
      // Some browsers wrap new lines in <div>. If it nests blocks, descend;
      // otherwise treat the whole div as a paragraph.
...
// Walk a set of sibling DOM nodes, grouping loose inline runs into paragraphs
// and converting block elements directly.
function flushHtmlChildren(nodes: ChildNode[], out: AstNode[]): void {
  ...
  const flush = () => {
    ...
    // Drop runs that are only whitespace (e.g. indentation between blocks).
...
/** Build a Hygraph `image` AST node from a media-library asset. */
export function assetToImageNode(asset: MediaAsset): AstNode {
  const node: AstNode = { type: "image", src: asset.url, children: [{ text: "" }] };
  if (asset.handle) node.handle = asset.handle;
  if (asset.width != null) node.width = asset.width;
  if (asset.height != null) node.height = asset.height;
  if (asset.mimeType) node.mimeType = asset.mimeType;
