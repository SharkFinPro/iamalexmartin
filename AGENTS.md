...
- **Inline rich-text editing**: for RichText fields, render `<RichTextWidget>` directly for visitors and `<EditableRichText model id field value={raw} />` (from `src/components/RichTextEditor`) only when `isAdmin` — keep visitors on the server-rendered widget (no client cost / SEO change). The query must fetch the entry `id` and `<field> { raw }`. The editor is a `contentEditable` surface (no editor deps; formatting via `execCommand`) that converts to/from Hygraph's AST in `richTextAst.ts`; images are inserted inline via `AssetPicker`, which reuses the Media Library's data (`listMediaAssets` → `getMediaAssets`). Inserted images are self-contained Hygraph `image` nodes (`src`/`handle`/dims/`mimeType`) so the default renderer shows them with no `references`. Writes go through `updateRichTextField` (whitelisted in `EDITABLE_RICH_TEXT_FIELDS`).
