...
- `src/app/admin/` — login/logout (`actions.ts`), the write Server Actions (`contentActions.ts`), a minimal dashboard (`page.tsx`, `DashboardControls.tsx`) for site-wide toggles, and a **Media Library** (`media/`) listing all Hygraph assets (any type, draft + published) with per-asset publish/rename/delete, multi-select **bulk** publish/unpublish/delete (delete is confirmed in a dialog first), and a **crop-and-upload** flow (`MediaUploader.tsx`, using `react-advanced-cropper`: a 2:1 preset matching how project images render, plus an always-available free-form mode). Uploads land as DRAFT; the admin publishes them with the existing button.
...
- `src/components/` — shared UI. Notably `EditableText/` (generic inline field editor), `RichTextEditor/` (admin-only inline rich-text editing — see below) and `useDragReorder.ts` (pointer-drag reorder hook used by both the projects grid and featured section).
...
- **Rich text lives inside the block system**: visitors get the server-rendered `<RichTextWidget>` (no client cost / SEO change); rich-text *editing* only exists as the `richText` block inside the block-layout editor (`RichTextEditor` in `src/components/RichTextEditor/RichTextEditor.tsx`). The editor is a `contentEditable` surface (no editor deps; formatting via `execCommand`) that converts to/from Hygraph's AST in `richTextAst.ts`; images are inserted inline via `AssetPicker`, which reuses the Media Library's data (`listMediaAssets` → `getMediaAssets`). Inserted images are self-contained Hygraph `image` nodes (`src`/`handle`/dims/`mimeType`) so the default renderer shows them with no `references`. There is no longer a standalone editable RichText *field* — content is authored as blocks (see below).
- **Block-layout pages**: project case-studies (`Project.projectPage`) and the About page (`RichTextWidget.blockLayout`) share one block system — `Block[]` types + `sanitizeProjectPage` validator in `src/components/ProjectBlocks/blocks.ts`, the `ProjectBlocks` visitor renderer, and the `ProjectPageEditor` admin surface (`{ entryId, title, initialBlocks, model?, field? }`). Persistence goes through `updateBlockLayout(model, id, field, blocks)` (whitelisted in `BLOCK_LAYOUT_FIELDS`); `updateProjectPage` is a thin wrapper. The query just fetches the entry `id` and the Json layout field; `sanitizeProjectPage` coerces null/invalid to `[]` so pages never crash.
...
- **Hygraph permissions**: the mutation token needs **Update (Draft) + Publish** on every editable model (`SiteConfig`, `Project`, `Description`, `PortfolioCard`, `RichTextWidget`), plus **Create + Read (Draft) + Update (Draft) + Publish + Unpublish + Delete** on `Asset` for the Media Library (upload, display, rename via the custom `title` field, publish/unpublish, and delete — deleting a published asset unpublishes it first). Uploads use Hygraph's direct-upload flow — `createAsset` returns a pre-signed S3 POST, then the binary is uploaded to storage (`cmsUpload` in `cms.ts`; the legacy `<host>/upload` endpoint is disabled on this project). The cropped image is exported client-side from a `<canvas>` and sent through the `uploadAsset` Server Action. A "Mutation failed due to permission errors" message means the token scope is missing, not a code bug.
